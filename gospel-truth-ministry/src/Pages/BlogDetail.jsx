  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="relative">
        {post.image ? (
          <div className="bg-center bg-cover h-72 md:h-96"
      ...
      <div className="max-w-4xl px-6 py-12 mx-auto">
        ...
        <article>
          {paragraphs.map((para, i) => (
            <p key={i}
               className={`text-gray-700 leading-relaxed text-base md:text-lg mb-6 ${fBody}`}>
              {para.replace(/\n/g, ' ')}
            </p>
          ))}
        </article>
        ...
        {relatedPosts.length > 0 && (
          <div className="pt-10 mt-16 border-t border-gray-200">
            <h2 className={`text-brand-900 text-2xl font-bold mb-8 ${fHeading}`}>
              {t('blog.related')}
            </h2>
              ...
              {relatedPosts.map(related => (
                <Link key={related.id} to={`/blog/${related.id}`}
                  ...
                  <h3 className={`text-gray-900 font-bold text-base leading-snug
                                 group-hover:text-brand-700 transition-colors ${fHeading}`}>
                    {related.title}
                  </h3>
